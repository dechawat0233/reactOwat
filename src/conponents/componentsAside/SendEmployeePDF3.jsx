  ...
  const generatePDF2 = () => {
    ...
    inputValuesFirst.forEach((value, index) => {
      if (index < 8) {
        ...
        const y =
          ...
          index * 15; // Y-coordinate, with 20 pixels separation for each item
        const y2 =
          72.5 +
          10 * (titleLines.length + inviteLines.length + contentLines.length) +
          index * 15; // Y-coordinate, with 20 pixels separation for each item
      ...
      if (
        ...
        autoContent4.forEach((line, index) => {
          ...
          doc.text(
            line,
            x,
            y + 10 * (autoContent2.length + autoContent3.length)
          );
          // doc.text(line, x, y + (5 * ((titleLines.length + inviteLines.length + contentLines.length) + lengthFirst * 3)));
        ...
        doc.text(
          ...
          15 +
            y +
            10 *
              (autoContent2.length + autoContent4.length + autoContent4.length)
        );
        doc.text(
          "(" + signature + ")",
          100 + x,
        ...
        autoContent2.forEach((line, index) => {
          ...
          doc.text(
            line,
            x,
            15 * inputValuesFirst.length +
              y +
              10 *
                (titleLines.length + inviteLines.length + contentLines.length)
          );
          // doc.text(line, x, y + (5 * ((titleLines.length + inviteLines.length + contentLines.length) + lengthFirst * 3)));
    ...
    arrayChunks.forEach((chunk, pageIndex) => {
      ...
      chunk.forEach((value, index) => {
        ...
        doc.text(`ตำแหน่ง: ${value.position}`, x + 60, y);
        doc.text(
          `ประวัติการศึกษา: ${value.educational}.${currentIndex + 1}`,
          x,
          y2
        );
          ...
          if (
            ...
            autoContent4.forEach((line, index) => {
              ...
              doc.text(
                line,
                x,
                y + yMultiplier * (autoContent2.length + autoContent3.length)
              );
            ...
            autoContent3.forEach((line, index) => {
              ...
              doc.text(
                line,
                x,
                y + 15 * chunk.length + yMultiplier * autoContent2.length
              );
            ...
            autoContent4.forEach((line, index) => {
              ...
              doc.text(
                ...
                y +
                  15 * chunk.length +
                  yMultiplier * (autoContent2.length + autoContent3.length)
              );
