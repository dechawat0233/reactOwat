...
function Compensation() {
    ...
    try {
      ...
      if (response.data.recordConclude.length < 1) {
        ...
        if (editStatus !== "") {
          ...
          // await setSumWorkHourX(response.data.recordConclude[0].concludeRecord.allTimes);
  ...
  useEffect(() => {
    if (!resultArrayWithWorkplaceRecords || loadStatus !== null) return;

    let ans = 0;
    let ans1 = 0;
    let ans2 = 0;

    const calculateWorkRate = (item) => {
      if (item.shift === "specialt_shift" && item.cashSalary === "") {
        return item.specialtSalary || 0;
      }
      return item.calculatedValue || item.workRate || 0;
    };

    const calculateWorkRateOT = (item, workRateOT2) => {
      if (commonNumbers123.has(parseInt(item.dates, 10))) {
        return item.calculatedValueOT
          ? `${item.calculatedValueOT} (${item.holidayOT})`
          : workRateOT2;
      }
      return item.calculatedValueOT
    ...
    const updatedDataTable = calculatedArray.map((item, index) => {
      ...
      const workRate = calculateWorkRate(item);

      const workRateOT2 =
        item.shift === "specialt_shift" && item.cashSalary === ""
          ? !isNaN(item.specialtSalaryOT)
            ? parseFloat(item.specialtSalaryOT).toFixed(2)
            : ""
          : !isNaN(item.workRate) &&
            !isNaN(item.workOfHour) &&
      ...
      const workRateOT = calculateWorkRateOT(item, workRateOT2);

      const addSalaryDay1 = !isNaN(addSalaryDay) ? addSalaryDay : "";

      if (!isNaN(workRate)) ans += parseFloat(workRate);
      if (!isNaN(workRateOT)) ans1 += parseFloat(workRateOT);
      if (!isNaN(addSalaryDay1)) ans2 += parseFloat(addSalaryDay1);
    ...
    setSumRateOT(ans1);
    setSumAddSalary(ans2);
    setDataTable(updatedDataTable.filter((row) => row !== null));
  }, [resultArrayWithWorkplaceRecords, calculatedArray, loadStatus]);
