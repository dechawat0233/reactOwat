          ...
          if (str1 > 20 && str1 <= lastday) {
            ...
            if (element.specialtSalary !== '' || element.specialtSalaryOT !== '') {
              ...
              if (specialDayOff1.includes(Number(str1))) {
                ...
                let workRateOT = ((parseFloat(wpResponse1.data.holidayOT) * (salary / 8)) * parseFloat(otTime));
                tmp.workRateOT = workRateOT || 0;
...
router.post('/autocreate', async (req, res) => {
  ...
  try {
    ...
    const searchEmp = { employeeId, name: '', idCard: '', workPlace: '' };
    const responseEmp = await axios.post(`${sURL}/employee/search`, searchEmp);
    const dataEmp = responseEmp.data;

    if (dataEmp?.employees?.length) {
      dataEmp.employees[0].addSalary.forEach(item => {
        if (item.roundOfSalary === 'daily') {
          addSalaryDaily.push(item);
        }
      });
    ...
    let prevMonth = String(prevMonthInt).padStart(2, '0');
    const lastday = new Date(year1, prevMonth, 0).getDate();

    const searchData1 = { employeeId, month: prevMonth, timerecordId: year1 };
    const response1 = await axios.post(`${sURL}/timerecord/searchemp`, searchData1);
    const data1 = response1.data;

    if (data1.recordworkplace?.length) {
      const wpId1 = dataEmp.employees[0].workplace || '';
      let salary = dataEmp.employees[0].salary || 0;

      const wGroup1 = await groupByWorkplaceId(data1.recordworkplace[0].employee_workplaceRecord);
      const wCalList1 = await getWorkplaceCalData(wGroup1, prevMonthInt, year1);

      for (const element of data1.recordworkplace[0].employee_workplaceRecord) {
        const tmp = processElement(element, wCalList1, salary, lastday, prevMonth, year1, sumWorkHour, sumWorkRate, sumWorkHourOt, sumWorkRateOt);
        concludeRecord.push(tmp);
      }
    }
    ...
    dataConclude.concludeRecord = concludeRecord;
    dataConclude.addSalary = addSalaryDaily;
    dataConclude.sumWorkHour = sumWorkHour;
    dataConclude.sumWorkRate = sumWorkRate;
    dataConclude.sumWorkHourOt = sumWorkHourOt;
    dataConclude.sumWorkRateOt = sumWorkRateOt;
    ...
    if (concludeRecord.length) {
      const newConclude = new conclude(dataConclude);
      await newConclude.save();
    }
...
async function getWorkplaceCalData(wGroup1, month, year) {
  const wCalList1 = [];
  for (const workplaceId of Object.keys(wGroup1)) {
    const group1 = wGroup1[workplaceId];
    const wpDataCalculator1 = { month, year, workplaceId: group1.workplaceId };

    try {
      const wpResponse1 = await axios.post(`${sURL}/workplace/caldata`, wpDataCalculator1);
      wCalList1.push({ 'workplaceId': group1.workplaceId, 'data': wpResponse1.data });
    } catch (error) {
      console.error(`Error processing workplace ID ${group1.workplaceId}:`, error);
    }
  }
...
function processElement(element, wCalList1, salary, lastday, month, year, sumWorkHour, sumWorkRate, sumWorkHourOt, sumWorkRateOt) {
  const tmp = {};
  const tmpWP = wCalList1.find(item => item.workplaceId === element.workplaceId);

  const workOfHour = (tmpWP?.data?.workOfHour) ?? 0;
  const workOfOT = parseFloat(tmpWP?.data?.workOfOT) ?? 0;
  const dayOff = tmpWP?.data?.workplaceDayOffList ?? [];
  const specialDayOff = tmpWP?.data?.specialDaylist ?? [];
  const dayOffCheck = [];
  ...
  if (str1 > 20 && str1 <= lastday) {
    ...
    // Further processing based on different conditions...

    // Update sumWorkHour, sumWorkRate, sumWorkHourOt, sumWorkRateOt accordingly...
  }

  return tmp;
}

function addMissingDays(concludeRecord, startDay, endDay, month, year) {
