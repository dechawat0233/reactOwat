...
const employeeoutsideSchema = new mongoose.Schema({
  employeeId: {
    type: String,
    required: true,
    unique: true
...
const Employee = mongoose.model('Employeeoutside', employeeoutsideSchema);
...
router.delete('/delete/:_id', async (req, res) => {
  try {
    ...
    const deletedEmployee = await Employee.findByIdAndDelete(employeeIdToDelete);
